...
basic_image_ptr file_format_png::optimize_try_drop_alpha(basic_image_ptr const& image)
{
...
void file_format_png::optimize_and_save(basic_image_ptr const& image, std::filesystem::path const& filename)
{
	save_image(optimize_try_drop_alpha(image), filename);
}

void file_format_png::save_image( basic_image_ptr const & image, std::filesystem::path const & filename )
{
	...
	for ( uint32_t y = 0; y < image->height; y++ )
		row_pointers[y] = image->pixels.get( ) + size_t(image->scanline) * y;
